...
from client.markups.specific import (
    ChangeNotificationsHour,
    NotificationsHourCallbackData,
    ChangeNotificationsMinute,
...
async def delete_password(callback: CallbackQuery, bot_control: BotControl):
    ...
    if await bot_control.api_status_code_processing(code, 200):
        await bot_control.update_text_message(
            Info(f"Password deleted {Emoji.KEY + Emoji.DENIAL}")
        )
...
async def input_email(callback: CallbackQuery, bot_control: BotControl):
    await bot_control.update_text_message(
        Input(
            f"{Emoji.EMAIL + Emoji.NEW} Enter new email", state=States.input_text_email
        )
    )
...
async def email_accept_input(message: Message, bot_control: BotControl):
    ...
    verify_code = await Mailing.send_verify_code(
        email, "Verify email", bot_control.storage.first_name
    )
...
async def email_accept_input(message: Message, bot_control: BotControl):
    ...
    if verify_code is None:
        ...
        bot_control.storage.verify_code = await Mailing.send_verify_code(
            bot_control.storage.email, "Verify email", bot_control.storage.first_name
        )
        await bot_control.update_text_message(
            Input(
    ...
    _, code = await bot_control.api.update_email(
        bot_control.storage.user_token, bot_control.storage.email
    )
    if await bot_control.api_status_code_processing(code, 201):
        await bot_control.update_text_message(
            Info(f"Email updated {Emoji.EMAIL + Emoji.OK}")
        )
...
async def delete_email(callback: CallbackQuery, bot_control: BotControl):
    ...
    if await bot_control.api_status_code_processing(code, 200):
        await bot_control.update_text_message(
            Info(f"Email deleted {Emoji.EMAIL + Emoji.DENIAL}")
        )
