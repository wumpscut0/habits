...
from frontend.markups.sign_in import SignUp, Nickname, Login, Password
...
async def open_sign_in(callback: CallbackQuery, state: FSMContext):
    ...
    sign_in: SignUp = await deserialize(current_data['sign_in'])
...
async def open_nickname(callback: CallbackQuery, state: FSMContext):
    ...
    sign_in: SignUp = await deserialize((await state.get_data())['sign_in'])
...
async def open_login(callback: CallbackQuery, state: FSMContext):
    ...
    sign_in_: SignUp = await deserialize((await state.get_data())['sign_in'])
