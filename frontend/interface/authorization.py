...
from frontend.interface import update_interface, close_session, refill_trash
...
async def open_authorization(callback: CallbackQuery, state: FSMContext, interface: Interface, message_id: int):
    message_id = await close_session(callback.message.chat.id, message_id)
    ...
    message = await callback.message.answer(text=await authorization.text, reply_markup=await authorization.markup)
